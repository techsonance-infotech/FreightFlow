...
export default function ConsignorsPage() {
  ...
  const fetchData = async () => {
    setLoading(true);
    try {
      const response = await fetch(`/api/v1/masters/consignors?page=${page}&search=${search}`);
  ...
  const handleDelete = async (item: Consignor) => {
    if (!confirm(`Delete consignor ${item.name}?`)) return;
    try {
  ...
  const columns = [
    ...
    { header: 'GSTIN', accessor: (row: Consignor) => row.gstin || '---', className: 'font-mono text-[10px] font-black text-blue-600' },
    { 
      header: 'Status', 
      accessor: (row: Consignor) => (
  ...
  return (
    <div className="space-y-8 animate-in fade-in duration-700">
      <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4">
        <div>
          <h1 className="text-3xl font-black tracking-tight text-slate-900">Consignor Master</h1>
          <p className="text-sm font-medium text-slate-500 mt-1">Manage sender entities and shipping origins</p>
        </div>
        <Button onClick={() => { setEditingItem(null); setIsModalOpen(true); }} icon="➕">Add Consignor</Button>
      </div>
      ...
      <Modal isOpen={isModalOpen} onClose={() => setIsModalOpen(false)} title={editingItem ? 'Edit Consignor' : 'Add New Consignor'} size="lg">
