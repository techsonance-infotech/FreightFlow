...
export default function DealerBillingPage() {
  const [loadType, setLoadType] = useState<'BOX' | 'PALLET' | 'BOTH'>('BOTH');
  ...
  const generatePDF = async () => {
    ...
    const dealerAddressLines = doc.splitTextToSize(dealer?.personName || '-', (boxWidth / 2) - 5);
    doc.text(dealerAddressLines, margin + 2, dealerInfoY);
    doc.text(`GST No.:- ${dealer?.gstin || '-'}`, margin + 2, dealerInfoY + (dealerAddressLines.length * 3.5) + 2);
    ...
    if (reportType === 'detailed') {
      const tableData = records.map((record, index) => {
        ...
        return [
          (index + 1).toString(),
          record.details[0]?.productName || 'Yarn',
          record.loadType === 'PALLET' ? 'Pallet' : (record.details[0]?.packingType || 'Box'),
          record.loadType === 'PALLET' ? `${record.totalBoxes} Nos` : `${Number(record.totalWeight).toFixed(2)} KG`,
          price.toFixed(2),
          amount.toFixed(2)
        ];
      });
      ...
      doc.setFontSize(9);
      doc.setFont('helvetica', 'normal');
      doc.text('Receiver\'s Signature:', margin + 2, currentY + 7); // Moved down from +5
      ...
      if (companyDetails?.signatureUrl) {
        try {
          ...
          if (sigData) {
            ...
            doc.addImage(sigData, 'PNG', pageWidth - margin - sw - 5, currentY + 12, sw, sh);
          }
        } catch (e) {}
      }
      ...
      doc.setFontSize(8);
      doc.setFont('helvetica', 'normal');
      doc.text('Authorised Signature', pageWidth - margin - 2, currentY + 30, { align: 'right' });
      ...
      const termsHeight = (termLines.length * 4) + 8;
    ...
    // Final Page Border - Strictly within margins
    doc.setDrawColor(200);
    ...
    <div className="space-y-6">
      <div className="flex flex-col md:flex-row md:items-center justify-between gap-4">
        <div>
          ...
          <p className="text-sm text-slate-500 dark:text-slate-400 mt-1">Generate consolidated invoices and detailed delivery reports for dealers.</p>
        </div>
        
        <div className="flex items-center gap-3">
          <Button variant="outline" className="rounded-xl border-slate-200 dark:border-slate-800" onClick={handleFetchRecords} disabled={isLoading}>
            <Filter className="h-4 w-4 mr-2" />
            Fetch Records
          </Button>
          <Button className="rounded-xl bg-blue-600 hover:bg-blue-700 shadow-lg shadow-blue-100 dark:shadow-none" onClick={generatePDF} disabled={records.length === 0}>
      ...
      <div className="grid grid-cols-1 xl:grid-cols-4 gap-6">
        ...
        <Card className="xl:col-span-1 p-6 rounded-3xl border-slate-200 dark:border-slate-800 shadow-sm bg-white dark:bg-slate-900 space-y-6 h-fit">
          <div className="space-y-4">
            <div className="space-y-2">
              <Label className="text-xs font-black uppercase tracking-widest text-slate-500">Party / Dealer</Label>
              <Select value={selectedDealerId} onValueChange={setSelectedDealerId}>
                <SelectTrigger className="rounded-xl border-slate-200 dark:border-slate-800 h-11 bg-slate-50 dark:bg-slate-900">
            ...
            <div className="space-y-2">
              ...
              <div className="flex items-center justify-between p-3 rounded-xl border border-slate-100 dark:border-slate-800 bg-slate-50 dark:bg-slate-900">
            ...
            <div className="space-y-4 pt-2">
              <div className="flex items-center gap-2">
                <Button 
                  ...
                  size="sm" 
                  className="flex-1 rounded-lg text-[10px] font-black h-8"
          ...
          <div className="space-y-6 pt-4 border-t border-slate-100 dark:border-slate-800">
            <div className="space-y-2">
              <label className="text-[10px] font-black uppercase tracking-[0.2em] text-slate-400 dark:text-slate-500 ml-1">GST Requirement</label>
              <div className="flex bg-slate-100 dark:bg-slate-900 rounded-full p-1 border border-slate-200 dark:border-slate-800">
                <button 
                  ...
                  className={cn(
                    ...
                    isGstRequired ? 'bg-blue-600 text-white shadow-lg shadow-blue-200 dark:shadow-blue-900/20' : 'text-slate-500 hover:text-slate-700 dark:hover:text-slate-400'
                  )}
                >
                  With GST
                </button>
                <button 
                  ...
                  className={cn(
                    "flex-1 px-4 py-2 rounded-full text-[10px] font-black uppercase tracking-widest transition-all", 
                    !isGstRequired ? 'bg-rose-600 text-white shadow-lg shadow-rose-200 dark:shadow-rose-900/20' : 'text-slate-500 hover:text-slate-700 dark:hover:text-slate-400'
                  )}
                >
                  Without GST
                </button>
            ...
            {isGstRequired && (
              <>
                <div className="space-y-2 animate-in fade-in slide-in-from-top-2">
                  <label className="text-[10px] font-black uppercase tracking-[0.2em] text-slate-400 dark:text-slate-500 ml-1">GST Type</label>
                  <div className="flex bg-slate-100 dark:bg-slate-900 rounded-full p-1 border border-slate-200 dark:border-slate-800">
                    <button 
                      type="button" 
                      ...
                      className={cn(
                        "flex-1 px-4 py-2 rounded-full text-[10px] font-black uppercase tracking-widest transition-all", 
                        gstType === 'intra' ? 'bg-blue-600 text-white shadow-lg shadow-blue-200 dark:shadow-blue-900/20' : 'text-slate-500 hover:text-slate-700 dark:hover:text-slate-400'
                      )}
                    >
                      Intra (CGST+SGST)
                    </button>
                    <button 
                      ...
                      className={cn(
                        "flex-1 px-4 py-2 rounded-full text-[10px] font-black uppercase tracking-widest transition-all", 
                        gstType === 'inter' ? 'bg-blue-600 text-white shadow-lg shadow-blue-200 dark:shadow-blue-900/20' : 'text-slate-500 hover:text-slate-700 dark:hover:text-slate-400'
                      )}
                    >
                      Inter (IGST)
                    </button>
                ...
                <div className="space-y-2 animate-in fade-in slide-in-from-top-2">
                  <label className="text-[10px] font-black uppercase tracking-[0.2em] text-slate-400 dark:text-slate-500 ml-1">GST Rate (%)</label>
                  <Select value={gstRate.toString()} onValueChange={(val) => setGstRate(Number(val))}>
                    <SelectTrigger className="w-full h-11 rounded-xl bg-slate-50 dark:bg-slate-900 border-none font-bold text-xs">
            ...
            <div className="space-y-2">
              ...
              <Select value={loadType} onValueChange={(val: any) => setLoadType(val)}>
                <SelectTrigger className="w-full h-11 rounded-xl bg-slate-50 dark:bg-slate-900 border-slate-200 dark:border-slate-800">
            ...
            {consolidatedItems.length > 0 && (
              <div className="space-y-3 pt-4 border-t border-slate-100 dark:border-slate-800 animate-in fade-in slide-in-from-bottom-4">
                ...
                <div className="space-y-3">
                  {consolidatedItems.map((item, idx) => (
                    <div key={idx} className="p-3 rounded-xl bg-slate-50 dark:bg-slate-900 border border-slate-100 dark:border-slate-800 space-y-2">
                      <div className="flex items-center justify-between">
                        <span className="text-[10px] font-black text-slate-700 dark:text-slate-300 truncate max-w-[120px]">{item.description}</span>
                        <span className="text-[9px] font-bold px-1.5 py-0.5 rounded bg-blue-100 text-blue-700 uppercase">{item.type}</span>
                      </div>
                      <div className="relative">
                        <IndianRupee className="absolute left-2.5 top-1/2 -translate-y-1/2 h-3 w-3 text-slate-400" />
                        <Input 
                          type="number" 
                          className="pl-8 h-8 rounded-lg text-xs bg-white dark:bg-slate-950" 
                          placeholder="Rate"
          ...
          <div className="space-y-4 pt-4 border-t border-slate-100 dark:border-slate-800">
             <div className="flex items-start gap-3 p-4 rounded-2xl bg-amber-50 dark:bg-amber-900/10 border border-amber-100 dark:border-amber-900/20">
        ...
        {/* Preview Area */}
        <Card className="xl:col-span-3 rounded-3xl border-slate-200 dark:border-slate-800 shadow-sm bg-white dark:bg-slate-900 overflow-hidden flex flex-col min-h-[600px]">
          <div className="p-6 border-b border-slate-100 dark:border-slate-800 flex items-center justify-between">
            <h3 className="font-black text-slate-900 dark:text-white flex items-center gap-2">
              {reportType === 'consolidated' ? <Layers className="h-5 w-5 text-indigo-500" /> : <TableIcon className="h-5 w-5 text-blue-500" />}
          ...
          <div className="flex-1 overflow-auto p-0">
            {records.length === 0 ? (
              <div className="h-full flex flex-col items-center justify-center text-slate-300 p-12 text-center">
                ...
                <h4 className="font-bold text-slate-500 dark:text-slate-400">No records to display</h4>
                <p className="text-xs mt-1 max-w-[200px]">Select a dealer and period then click "Fetch Records" to start.</p>
              </div>
            ) : reportType === 'detailed' ? (
              <>
                <div className="overflow-x-auto max-h-[500px] overflow-y-auto relative border-b border-slate-100 dark:border-slate-800">
                  <table className="w-full text-left border-collapse">
                    ...
                    <tbody className="divide-y divide-slate-50 dark:divide-slate-800">
                      {records
                        ...
                        .map((record) => (
                        <tr key={record.id} className="border-b border-slate-50 dark:border-slate-800 hover:bg-slate-50/50 dark:hover:bg-slate-800/50 transition-colors">
                          <td className="px-6 py-4 text-xs font-bold text-slate-600 dark:text-slate-300">{format(new Date(record.date), 'dd MMM yyyy')}</td>
                          <td className="px-6 py-4 text-xs font-black text-slate-900 dark:text-white">{record.lrNo}</td>
                          <td className="px-6 py-4">
                            <div className="flex flex-col">
                              <span className="text-xs font-bold text-slate-900 dark:text-white">{record.details[0]?.productName || 'Yarn'}</span>
                          ...
                          <td className="px-6 py-4 text-xs font-black text-slate-900 dark:text-white">
                            {record.loadType === 'PALLET' ? `${record.totalBoxes} Nos` : `${record.totalWeight} Kg`}
                          </td>
                ...
                {records.length > itemsPerPage && (
                  <div className="px-6 py-4 flex items-center justify-between bg-white dark:bg-slate-950 border-t border-slate-100 dark:border-slate-800">
                ...
                <div className="p-8 bg-slate-50 dark:bg-slate-900/50 flex flex-col md:flex-row justify-between gap-8 border-t border-slate-100 dark:border-slate-800">
                  <div className="flex flex-col gap-4">
                    <h4 className="text-[10px] font-black uppercase tracking-widest text-slate-400 border-b border-slate-200 dark:border-slate-800 pb-2">Load Type Breakdown</h4>
                    <div className="flex flex-wrap gap-12">
                      ...
                      {records.some(r => r.loadType === 'PALLET') && (
                        <div>
                          <span className="text-[10px] font-black text-slate-400 uppercase block mb-1">Pallet Total (Qty)</span>
                          <div className="flex flex-col">
                            <span className="text-xl font-black text-slate-900 dark:text-white">
                              {records.filter(r => r.loadType === 'PALLET').reduce((acc, r) => acc + (r.totalBoxes || 0), 0)} Nos
                            </span>
                            <span className="text-base font-black text-orange-600 mt-1">
                              ₹ {records.filter(r => r.loadType === 'PALLET').reduce((acc, r) => {
                                const prodName = r.details[0]?.productName || 'Pallet';
                                const packType = 'Pallet';
                                const itemKey = `${prodName}-${packType}`;
                                const price = consolidatedItems.find(i => `${i.description}-${i.type}` === itemKey)?.unitPrice || 0;
                  ...
                  <div className="flex flex-col items-end gap-3">
                    ...
                    <div className="h-[1px] w-full max-w-[300px] bg-slate-200 dark:bg-slate-800 my-2" />
              ...
              <div className="p-6 space-y-4">
                <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                  {consolidatedItems.map((item, idx) => (
                    <div key={idx} className="p-5 rounded-2xl border border-slate-100 dark:border-slate-800 bg-white dark:bg-slate-900/50 hover:border-blue-200 dark:hover:border-blue-900 transition-all shadow-sm">
                      <div className="flex items-center justify-between mb-4">
                        <div className="flex items-center gap-3">
                          ...
                          <div>
                            <h4 className="text-sm font-black text-slate-900 dark:text-white">{item.description}</h4>
                      ...
                      <div className="grid grid-cols-2 gap-4">
                        <div className="p-3 rounded-xl bg-slate-50 dark:bg-slate-900 border border-slate-100 dark:border-slate-800">
                          ...
                          <span className="text-sm font-black text-slate-900 dark:text-white">{item.totalWeight.toFixed(2)} KG</span>
                        </div>
                        <div className="p-3 rounded-xl bg-slate-50 dark:bg-slate-900 border border-slate-100 dark:border-slate-800">
                          <span className="text-[9px] font-black text-slate-400 uppercase block">Total Quantity</span>
                          <span className="text-sm font-black text-slate-900 dark:text-white">{item.totalQty} Nos</span>
                        </div>
                      </div>
                    </div>
                  ))}
                ...
                <div className="p-8 bg-slate-50 dark:bg-slate-900/50 flex flex-col md:flex-row justify-between gap-8 border-t border-slate-100 dark:border-slate-800">
                  ...
                  <div className="flex flex-col gap-4">
                    <h4 className="text-xs font-black uppercase tracking-widest text-slate-400 border-b border-slate-200 dark:border-slate-800 pb-2">Load Type Breakdown</h4>
                    <div className="flex flex-wrap gap-12">
                      ...
                      {consolidatedItems.filter(i => i.loadType === 'PALLET').length > 0 && (
                        <div>
                          <span className="text-[10px] font-black text-slate-400 uppercase block mb-1">Pallet Total (Qty)</span>
                          <span className="text-lg font-black text-slate-900 dark:text-white block">
                            {consolidatedItems.filter(i => i.loadType === 'PALLET').reduce((acc, i) => acc + i.totalQty, 0)} Nos
                          </span>
                          <span className="text-sm font-bold text-orange-600 block mt-1">
                            ₹ {consolidatedItems.filter(i => i.loadType === 'PALLET').reduce((acc, i) => acc + (i.totalQty * i.unitPrice), 0).toLocaleString('en-IN', { minimumFractionDigits: 2 })}
                          </span>
                        </div>
                      )}
                    </div>
                  ...
                  {isGstRequired && (
                    <div className="flex items-center gap-10">
                      <span className="text-[10px] font-black uppercase tracking-widest text-indigo-500">Total GST</span>
                      <span className="text-lg font-black text-indigo-600">
