...
export default function AccountsReceivablePage() {
  ...
  const fetchAR = async () => {
    setLoading(true);
    try {
      const response = await fetch('/api/v1/accounting/reports/ageing?type=AR');
  ...
  return (
    <div className="space-y-8 animate-in fade-in duration-700">
      <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4">
        <div>
          <h1 className="text-3xl font-black tracking-tight text-slate-900">Accounts Receivable</h1>
          <p className="text-sm font-medium text-slate-500 mt-1">Manage outstanding customer dues and ageing</p>
        </div>
        <div className="flex items-center gap-3">
          <Button onClick={fetchAR} variant="outline" icon="🔄">
            Refresh
          </Button>
        </div>
      </div>
                  ...
                  {data.items.length === 0 ? (
                    <tr>
                      <td colSpan={5} className="px-6 py-12 text-center">
                        <p className="text-sm font-bold text-slate-400 uppercase tracking-widest">No outstanding invoices</p>
                      </td>
                    </tr>
                  ) : (
                    data.items.map((inv, idx) => (
                      <tr key={idx} className="hover:bg-slate-50 transition-colors">
                        ...
                        <td className="px-6 py-4 text-sm font-bold text-slate-600">{inv.customer?.name || 'Unknown'}</td>
