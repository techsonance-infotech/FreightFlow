...
async function renderCopy(doc: jsPDF, pallet: any, company: any, copyTitle: string, startY: number) {
  ...
  doc.text(`Date: ${formatUtcDate(pallet.date, 'dd/MM/yyyy')}`, pageWidth - margin - 2, brandY + 8.5, { align: 'right' });
  ...
  if (dCode) {
    ...
    doc.setFont('helvetica', 'bold');
    doc.text(` - ${dCode}`, Math.min(margin + 2 + dNameWidth, pageWidth / 2 - 10), partyY + 3.8);
  }
  ...
  doc.setFontSize(9.5);
  doc.text(cName.toUpperCase(), pageWidth / 2 + 2, partyY + 3.8);
  if (cCode) {
    const cNameWidth = doc.getTextWidth(cName.toUpperCase());
    doc.setFont('helvetica', 'bold');
    doc.text(` - ${cCode}`, Math.min(pageWidth / 2 + 2 + cNameWidth, pageWidth - margin - 10), partyY + 3.8);
  }
