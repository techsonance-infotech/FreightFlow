      ...
      <div className="bg-white w-full max-w-4xl h-full max-h-[90vh] rounded-[2.5rem] shadow-2xl flex flex-col overflow-hidden animate-in zoom-in-95 duration-300">
        ...
        <div className="flex-1 overflow-y-auto p-12 bg-slate-50/50" id="printable-lr">
          <div className="space-y-12">
            {items.map((item, idx) => (
              <div key={idx} className="bg-white border border-slate-200 shadow-sm p-16 rounded-[2rem]">
                {/* Template Header */}
                <div className="flex justify-between items-start border-b border-slate-200 pb-10 mb-10">
                  <div>
                    ...
                    <div className="mt-6 space-y-1">
                      <p className="text-[10px] font-bold text-slate-400 uppercase">Head Office</p>
                  ...
                  <div className="text-right">
                    ...
                    <div className="bg-slate-50 border border-slate-200 text-slate-800 px-6 py-3 rounded-2xl inline-block mb-4">
                      <p className="text-[10px] font-bold uppercase tracking-widest text-slate-400">Lorry Receipt No.</p>
                      <p className="text-2xl font-black tracking-tighter text-slate-900">#{item.lrNo}</p>
                    </div>
                    <div className="space-y-1">
                      <p className="text-[10px] font-bold text-slate-400 uppercase">Date</p>
                      <p className="text-sm font-bold text-slate-900 uppercase tracking-tighter">{formatUtcDate(item.date, 'dd MMMM yyyy')}</p>
                    </div>
                  </div>
                </div>
                ...
                <div className="grid grid-cols-2 gap-12 mb-12">
                  <div className="space-y-4">
                    <div className="flex items-center gap-2 mb-2">
                      ...
                      <h3 className="text-[10px] font-bold uppercase tracking-widest text-slate-400">Consignor / Dealer</h3>
                    </div>
                    <div>
                      <p className="text-lg font-bold text-slate-900 uppercase tracking-tight">{item.dealer?.name}</p>
                  ...
                  <div className="space-y-4">
                    <div className="flex items-center gap-2 mb-2">
                      ...
                      <h3 className="text-[10px] font-bold uppercase tracking-widest text-slate-400">Consignee</h3>
                    </div>
                    <div>
                      <p className="text-lg font-bold text-slate-900 uppercase tracking-tight">{item.consignee?.name}</p>
                ...
                <div className="grid grid-cols-3 gap-6 mb-12 bg-slate-50 p-6 rounded-3xl border border-slate-100">
                  <div>
                    <p className="text-[9px] font-bold text-slate-400 uppercase tracking-widest mb-1">Vehicle No</p>
                    <div className="flex items-center gap-2">
                      <Truck className="h-4 w-4 text-blue-600" />
                      <p className="font-bold text-slate-900 uppercase tracking-tighter">{item.vehicle?.plateNumber || item.vehicle?.regNo || 'N/A'}</p>
                    </div>
                  </div>
                  <div>
                    <p className="text-[9px] font-bold text-slate-400 uppercase tracking-widest mb-1">From</p>
                    <p className="font-bold text-slate-900 uppercase tracking-tighter truncate">{item.fromLocation}</p>
                  </div>
                  <div>
                    <p className="text-[9px] font-bold text-slate-400 uppercase tracking-widest mb-1">To</p>
                    <p className="font-bold text-slate-900 uppercase tracking-tighter truncate">{item.toLocation}</p>
                  </div>
                </div>
                ...
                <div className="mb-12">
                  <table className="w-full text-left">
                    <thead className="bg-slate-50 border-b border-slate-200">
                      <tr>
                        <th className="py-3 px-4 text-[10px] font-bold uppercase tracking-widest text-slate-600">Description of Goods</th>
                        <th className="py-3 px-4 text-[10px] font-bold uppercase tracking-widest text-slate-600 text-center">Packing</th>
                        <th className="py-3 px-4 text-[10px] font-bold uppercase tracking-widest text-slate-600 text-center">Units</th>
                        <th className="py-3 px-4 text-[10px] font-bold uppercase tracking-widest text-slate-600 text-right">Weight (KG)</th>
                      </tr>
                    </thead>
                    <tbody className="divide-y divide-slate-100">
                      {(item.details || []).map((detail: any, i: number) => (
                        <tr key={i}>
                          <td className="py-4 px-4 text-xs font-semibold text-slate-700 uppercase">{detail.productName}</td>
                          <td className="py-4 px-4 text-xs font-medium text-slate-500 text-center">{detail.packingType}</td>
                          <td className="py-4 px-4 text-xs font-bold text-slate-800 text-center">{detail.boxCount}</td>
                          <td className="py-4 px-4 text-xs font-bold text-slate-800 text-right">{formatWeight(detail.weight)}</td>
                        </tr>
                      ))}
                    </tbody>
                  </table>
                ...
                <div className="mt-24 grid grid-cols-2 gap-24">
                  <div className="border-t border-slate-200 pt-6">
                    <p className="text-[10px] font-bold uppercase tracking-widest text-slate-400 text-center">Consignor Signature</p>
                  </div>
                  <div className="border-t border-slate-200 pt-6">
                    <p className="text-[10px] font-bold uppercase tracking-widest text-slate-800 text-center">For FreightFlow Logistics</p>
                  </div>
                </div>
              </div>
            ))}
